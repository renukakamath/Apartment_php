...
public class Sendcomplaint extends AppCompatActivity implements  JsonResponse{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String q ="?action=complaintview&login_id="+sh.getString("log_id","" );
        q = q.replace(" ","%20");
        JR.execute(q);
        ...
        b1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                JsonReq JR = new JsonReq();
                JR.json_response = (JsonResponse) Sendcomplaint.this;
                String q = "?action=complaint&complaint="+complaint+"&login_id="+sh.getString("log_id","" );
                q = q.replace(" ","%20");
                JR.execute(q);
            }
        });
    ...
    public void response(JSONObject jo) {
        try {
            ...
            else if(method.equalsIgnoreCase("complaintview"))
            {
                String status=jo.getString("status");
                Log.d("pearl",status);
